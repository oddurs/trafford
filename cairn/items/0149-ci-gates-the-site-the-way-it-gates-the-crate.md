...
- **Build it.** `site build` with warnings as errors. Broken internal links
  already fail the build per #0166; CI is where that is enforced on a
...
- **Check external links, on a schedule and not on every pull request.** They
  ...
  shape.
- **Assert the generated things are current** — the palette from #0147, the
  screenshots from #0148. Regenerate, `git diff --exit-code`, fail with the
  command to run.
